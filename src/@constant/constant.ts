export const TOKEN_USER = "token";
export const EMAIL_USER = "email";
export const ADDRESS_USER = "address";
export const COMPANY_USER = "company_name";
export const NAME_USER = "name";
export const NPWP_USER = "npwp";

export const ID_USER = "id";

export const ADD_PATH = "/add";
export const EDIT_PATH = "/edit";
export const EDIT_DRAFT_PATH = "/edit-draft";
export const VIEW_PATH = "/view";

export const locale = "en-us";

export const FORMAT_DATE_1 = "yyyy-MM-DD";
export const FORMAT_DATE_1_TIME = "yyyy-MM-DD HH:mm:ss";
export const FORMAT_DATE_2 = "DD MMM yyyy";
export const FORMAT_DATE_3 = "yyyy-MM";
export const FORMAT_DATE_TIME = "DD MMM yyyy, HH:mm";

export const DEFAULT_ERROR_MESSAGE = "Ooops something wrong!";
export const INTERNAL_SERVER_ERROR = "Internal Server Error";
export const UNAUTHORIZED = "unauthorized";
export const UNAUTHORIZED_MESSAGE = "It looks like you're not logged in. Reload the page to try again";

export const FILE_TYPE_ERROR = "file type not support!";
export const FILE_SIZE_ERROR = "file size too big or too small!";

export const AWS_PATH = "https://s3.ap-southeast-1.amazonaws.com";

export const DANGER_COLOR = "#E61537";
export const SUCCESS_COLOR = "#3F8600";
export const PRIMARY_COLOR = "#1A73E8";

export const KEY_UPLOAD_FILE = "upload-file-";

export const IMAGE_FALLBACK = `data:image/png;base64,`;

export const ACTION_PATHS = [ADD_PATH, EDIT_PATH, VIEW_PATH];

export const AUTOGENERATED = "Autogenerated";
