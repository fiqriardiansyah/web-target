...
export default function SearchCustomerSales({ ...props }: SearchCustomerSalesProps) {
        ...
        <ModalCustom {...props} handlerInComponent={props.children} footer={null}>
            {(ctrl) => (
                <div className="pt-3">
                    ...
                    {pendingQuery.data?.map((order, i) => (
                        <>
                            <OrderCardSales order={order} refreshPendingOrder={pendingQuery.refetch} key={i} />
                            <div className="w-full h-[1px] bg-gray-200 mt-2 mb-4"></div>
                        </>
                    ))}

                    <Button disabled={!customer} type="primary" className="w-full mt-5" size="large">
                        Create New Order
                    </Button>
                </div>
            )}
